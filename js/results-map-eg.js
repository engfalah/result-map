...
// Analytics
  // var _gaq = _gaq || [];
  // _gaq.push(['_setAccount', 'UA-31682710-1']);
  // _gaq.push(['_setDomainName', 'egypt-election-2012-demo.appspot.com']);
...
//		}).join( ',' ),
//		',ID,ValidVotesCount',
//		',NumDistricts,NumDistrictsReported'
//	);
//}
...
document.write(
	'<style type="text/css">',
		...
		'#topbar, #sidebar, #maptip, .tiptitletext { font-family: Tahoma, Traditional Arabic, Arial, sans-serif; font-size: ', opt.fontsize, '; background-color:white; }',
		...
		'div.title-text { font-size:20px; font-family: Traditional Arabic, Arial, sans-serif; }',
		'div.subtitle-text { font-size:11px; color:#777; margin: 10px 0; }',
		'div.body-text, div.body-text label { font-size:13px; }',
		'div.faint-text { font-size:10px; color:#777; }',
		...
		'#sidebar { width:', sidebarWidth, 'px; float: right; }',
		'#sidebar table.candidates { width:100%; border: 1px solid #CCC; }',
		'table.candidates td { border-top:1px solid #E7E7E7; padding: 3px;}',
		'#maptip table.candidates { width:100%; font-size: 13px; }',
...
(function( $ ) {
  ...
  function addLivingAbroad( features ) {
    var center = { x: 3235000, y: 3830000 };
    var radius = 30000;
    var steps = 32;
    var feature = {
      bbox: [
  ...
  function drawCircle( center, radius, steps ) {
	...
	var players = {
		...
		counties: {
			...
			//		var use = row && row[col.NumDistrictsReported];
			//		if( use ) {
...
*/
	...
	function colorVotes( features, strokeColor, strokeOpacity, strokeWidth ) {
		...
		if( !( candidates && currentCandidate ) ) {
			for( var iFeature = -1, feature;  feature = features[++iFeature]; ) {
				...
				var complete = row &&
					row[col.NumDistrictsReported] ==
					row[col.NumDistricts];
				feature.strokeColor = strokeColor;
				feature.strokeOpacity = strokeOpacity;
				feature.strokeWidth = strokeWidth;
			}
	...
	function formatCandidateIcon( candidate, size ) {
		...
		if( candidate.id ) {
			...
			photo = S(
				'background:url(',
					imgUrl( S( 'candidate-photos-eg-', params.year, '-', size, '.png' ) ),
				'); ',
				'background-position:-',
	...
	function totalReporting( results ) {
		...
		for( var row, i = -1;  row = rows[++i]; ) {
			counted += row[col.NumDistrictsReported];
			total += row[col.NumDistricts];
		}
		return {
			counted: counted,
			total: total,
	...
	function getTopCandidates( results, result, sortBy, max ) {
		...
		for( var i = -1;  ++i < top.length; ) {
			...
			candidate.vsAll = votes / result[col.ValidVotesCount];
	...
	function getCandidateDelegates( state, candidate ) {
		...
		var iCol = delegates.colsById[ 'VoteCount-' + candidate.id ];
		var row =
	...
	function makeCurrentCandidateValid() {
		if( ! currentCandidate )
			return;
		var results = geoResults();
		var col = results.colsById[ 'VoteCount-' + currentCandidate ];
	...
	function formatSidebar() {
		...
		if( results ) {
			...
			var viewNational = nationalEnabled() ? S(
			  '<div style="font-size:10px;">',
  				'<a href="#" id="viewNational" title="', 'titleViewNational'.T(), '" style="">',
  					'viewNational'.T(),
  				'</a>',
  		  '</div>'
			) : '';
        ...
        '<div id="percent-reporting" class="body-text">',
          current.national ? 'percentReporting'.T( totalReporting(results) ) : '',
        '</div>'
			);
	...
	function formatCandidateList( topCandidates, formatter, header ) {
		...
		var thead = header ? S(
			// '<tr>',
				// '<th colspan="3" style="padding-bottom:4px;">',
					// 'candidate'.T(),
				// '</th>',
	...
	function mayHaveResults( row, col ) {
		return(
			row[col.ValidVotesCount] > 0  ||
			row[col.NumDistrictsReported] < row[col.NumDistricts]
		);
	}
	...
	function formatTip( where ) {
		...
		if( row  &&  col  &&  mayHaveResults(row,col) ) {
			row.geoid = geoid;
			row.geo = geo;
			top = getTopCandidates( results, row, 'votes' );
			var content = S(
			...
			var boxes = row[col.NumDistricts];
			var counted = row[col.NumDistrictsReported];
		}
	...
	function resizeViewOnly() {
		...
		var mapRight = sidebarWidth, mapTop = topbarHeight;
		var mapWidth = ww - mapRight, mapHeight = wh - mapTop;
	...
	function getElections( electionids ) {
      ...
      $.ajax({
        url: "votes/" + current.geoid + "-round-1.js",
        dataType: "json",
        cache: true,
        statusCode: {
          404: function() {
            loadResults(empty_result, electionid, "live");
          }
        },
        success: function(result){
          loadResults(result, electionid, "live");
        }
      });
		});
	}
	...
	function loadTestResults( electionid, randomize ) {
		...
		col = col.concat('ID', 'ValidVotesCount', 'NumDistricts','NumDistrictsReported');
		col.index();
		...
		var rows = currentGeo().features.map( function( feature ) {
			...
			var nVoters = 0;
			var nPrecincts = row[col.NumDistricts] = random( 50 ) + 5;
			var nCounted = row[col.NumDistrictsReported] =
				Math.max( 0,
					Math.min( nPrecincts,
						random( nPrecincts * 2 ) -
			...
			row[col.ValidVotesCount] = total + random(total*2);
			return row;
		});
		
		 // var rows = [ 
                  // [ 20, 10, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, '01', 910, 10000, 8000 ],
                  // [ 130, 20, 30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 10, '02', 910, 7000, 3500 ] 
                // ]
	...
	function loadResultTable( json, loading ) {
		...
		for( var i = 0, colID = col.ID;  i < colID;  ++i ) {
			var id = cols[i].replace(/^VoteCount-/,'').toLowerCase(),
		...
		for( var row, iRow = -1;  row = rows[++iRow]; ) {
			...
			if( zero ) {
				...
				row[col.ValidVotesCount] = 0;
				totals[col.NumDistricts] += row[col.NumDistricts];
				row[col.NumDistrictsReported] = 0;
			}
			else {
				for( iCol = -1;  ++iCol < nCandidates; ) {
					var count = row[iCol];
				...
				totals[col.ValidVotesCount] += row[col.ValidVotesCount];
				totals[col.NumDistricts] += row[col.NumDistricts];
				totals[col.NumDistrictsReported] += row[col.NumDistrictsReported];
			}
